...
class Cart {
        ...
        if (this.items.length === 0) {
            ...
            this.items.forEach(item => {
                ...
                cartItemElement.innerHTML = `
                    <div class="cart-item-image" style="background-color: #f0f0f0; display: flex; align-items: center; justify-content: center;">
                        <i class="fas fa-box"></i>
                    </div>
                    <div class="cart-item-details">
                        <div class="cart-item-title">${item.name}</div>
                        <div class="cart-item-price">${item.price} ل.س</div>
    ...
    sendOrderViaWhatsApp() {
        ...
        this.items.forEach((item, index) => {
            message += `${index + 1}. ${item.name} - ${item.quantity} × ${item.price} ل.س = ${item.quantity * item.price} ل.س\n`;
        });
        
        message += `\nالمجموع الكلي: ${this.getTotal()} ل.س\n\n`;
